...
import com.media.doopy.Activity.RecipeInfo;
import com.media.doopy.Controller.Component;
import com.media.doopy.CoreData.Recipe.Hits;
import com.media.doopy.CoreData.Recipe.Ingredients;
    ...
    public class ContentAdapter extends RecyclerView.Adapter<ContentAdapter.ViewHolder> {
        ...
        class ViewHolder extends RecyclerView.ViewHolder {
            ...
            void Click(){
                ...
                Hits h = ((Hits)hits.get(getAdapterPosition()));

                Intent i = new Intent(getContext(), RecipeInfo.class);

                i.putExtra(RecipeInfo.KEY_LABEL, ((Recipe)h.getRecipe()).getLabel());

                String ingredientes = "";
                for (Component ing:((Recipe)h.getRecipe()).getIngredients()) {
                    ingredientes += "a) " + ((Ingredients)ing).getText() + "\n";
                    ingredientes += "--> " + ((Ingredients)ing).getWeight() + "\n";
                }
                i.putExtra(RecipeInfo.KEY_INGRE, ingredientes);

                i.putExtra(RecipeInfo.KEY_CALO, "" + ((Recipe)h.getRecipe()).getCalories());

                i.putExtra(RecipeInfo.KEY_PESO_TOT, "" + ((Recipe)h.getRecipe()).getTotalWeight());

                i.putExtra(RecipeInfo.KEY_IMAGE, "" + ((Recipe)h.getRecipe()).getImage());

                // Get the transition name from the string
                Pair<View, String> p1 = Pair.create((View)img, getString(R.string.topImage));
                Pair<View, String> p2 = Pair.create((View) titulo, getString(R.string.topTitle));
